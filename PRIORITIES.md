...
- **NO `as` type assertions** — Use runtime validation (zod schemas) or typed helper functions instead. Every `as` cast is a bug waiting to happen. ⚠️ **IN PROGRESS**: Eliminated 4 unsafe type assertions with proper validation (ProviderId validation in agents, Provider validation in credentials, unnecessary project config assertion). Remaining: ~59 type assertions to address.
...
*Last read by Leopard: 2026-02-13 (Cycle 11)*
